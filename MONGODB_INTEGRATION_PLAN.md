...
| Security fix (2026-09-22): vector store secret redirects | One rule now closes the class: only proxy admins may set or change where a vector store sends traffic. `VECTOR_STORE_ENDPOINT_KEYS` (api_base, base_url, endpoint, azure_endpoint, azure_search_service_name, aws_bedrock_runtime_endpoint, aws_sts_endpoint, aws_region_name, aws_role_name, aws_profile_name, aws_session_name, aws_external_id, aws_web_identity_token, tenant_id, client_id, vertex_project/vertex_ai_project, vertex_location/vertex_ai_location, valkey_host, valkey_port, valkey_ssl) plus `litellm_credential_name` are admin-only anywhere in litellm_params on `/vector_store/new`, `/vector_store/update` and RAG ingest (403 to set, change or clear; keeping a saved value is fine), and a non-admin can't change a store's provider. A managed store's saved endpoints are pinned on every search, RAG query and ingest: the store's value, else its credential's, else None so the provider default applies and the request body can't fill it. Search and RAG query bodies from non-admins can't carry those keys either, and `azure_search_service_name`/`azure_endpoint` joined the proxy-wide request-body ban list. Closes: a non-admin store with its own api_base and no key receiving the proxy's env key (OPENAI_API_KEY, AZURE_API_KEY, ...) on create or api_base update; `litellm_embedding_config.api_base` redirecting a router deployment's key; the credential-endpoint drop letting the body fill api_base or azure_search_service_name; stores pairing a credential with an admin-saved endpoint (azure_ai, pg_vector, milvus, ragflow, per-store MongoDB sidecar) losing it; and aws_role_name/aws_profile_name/tenant_id/client_id being editable by non-admins. Non-admins keep creating, using and searching stores on admin-set endpoints or provider defaults. `company_handbook` (admin-saved sidecar api_base, `os.environ/MONGODB_SIDECAR_API_KEY`) is unaffected. Open: `is_request_body_safe` does not look inside `litellm_params.litellm_embedding_config`, since it runs for admins too on `/vector_store/new`, where that nested api_base is legitimate; the management routes cover it instead |
