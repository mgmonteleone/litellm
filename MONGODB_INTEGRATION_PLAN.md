...
| PR 5 (dashboard) | All five sections implemented on branch `feat/mongodb-dashboard` (`33357d9f`, `61bc810c`, `de0f04c9`, `9c968592`, `3430eae7`). 5.1 add dialog: setup alert with a copyable docker run, Test connection rendering the proxy checklist with per-row fixes, Show details and a secret-masking copy-as-curl, discovery-fed database/collection/field pickers with plain text fallback on a 403, an existing-vs-new index section, an inline embedding dimension check, and an advanced group whose hybrid controls appear only when the sidecar reports $rankFusion. 5.2 ingest: the `embedding_model` to `litellm_embedding_model` rename (MongoDB ingest was failing without it), the embedding combobox, chunking controls, ingest counts and a Test it link. 5.3 info page: Connection card with redacted params, Test connection by store id, index status chip, Files card, an edit form no longer filtered to Bedrock, and an inline Edit connection flow on the Connection card itself that reuses the add dialog's MongoDB and generic field components, prefills from the saved (redacted) params, sends only the changed keys plus untouched secrets as the exact redaction sentinel, persists an `os.environ/` reference verbatim, and offers Test connection before saving. 5.4 test tab: options bar, AND/OR filter builder keyed from `mongodb_filter_fields`, score bar, attribute chips, hybrid contributions, copy as curl/Python. 5.5 capability chips from a static map. 253 dashboard tests pass across 25 files; eslint clean with every lint budget unchanged. Remaining: per-file delete on the Files card (needs the documents delete endpoint wired through the proxy), a live index status poll rather than one derived from the last Test connection, and chunk/vector counts in the ingest alert, which `RAGIngestResponse` does not carry yet |
| Deployment-configured MongoDB sidecar defaults (backend fallback, `GET /vector_store/provider_defaults`, dashboard) | Implemented: `get_complete_url` falls back to `MONGODB_SIDECAR_API_BASE` the same way `api_key` already falls back to `MONGODB_SIDECAR_API_KEY`, so search, create, ingest, and diagnostics all resolve the sidecar URL through the one place that validates it; the error names both `api_base` and `MONGODB_SIDECAR_API_BASE` when neither is set. New admin-only `GET /vector_store/provider_defaults` reports the configured `api_base` and whether an `api_key` is set, never the key itself. The add dialog and the info page's edit connection flow fetch it on mount and, when the deployment has both configured, show "Using this deployment's MongoDB sidecar at &lt;api_base&gt;" and collapse Sidecar URL/API Key behind an "Override sidecar connection" toggle that, collapsed, sends neither field |
...
| Security fix (2026-09-22): saved `os.environ/` references leaked proxy secrets | `4b446c7a` (deployed as `abd122b3` on `deploy/mongodb-v2-1.101.0`) started resolving `os.environ/NAME` in saved vector store params at use time, while `/vector_store/new`, `/vector_store/update` and RAG ingest let any key with vector store access save them. A store with `api_key: os.environ/LITELLM_MASTER_KEY` (or `DATABASE_URL`, `MONGODB_SIDECAR_API_KEY`, any provider key) and its own `api_base` got that secret sent to the chosen host on the next search, ingest, chat tool call or test connection. The exposure existed in every deployed image built from `4b446c7a`/`abd122b3` until this fix. Fixed on `feat/mongodb-vector-store-v2` in three commits: saving or changing params that hold a reference now needs a proxy admin (403 otherwise); saved references resolve only for allowlisted `provider:NAME` pairs (`mongodb:MONGODB_SIDECAR_API_KEY` built in, more via `LITELLM_VECTOR_STORE_ENV_REFERENCE_ALLOWLIST`), other references reach the provider literally; and the MongoDB provider sends a key equal to `MONGODB_SIDECAR_API_KEY` only to the deployment's own sidecar, however the key arrived. Config-registered stores are unaffected since the config file is resolved at load. Follow-up: rotate `LITELLM_MASTER_KEY`, the sidecar key and provider keys on the deployment, and check existing `LiteLLM_ManagedVectorStoresTable` rows for `os.environ/` values |
| Security fix (2026-09-22): vector store secret redirects | One rule now closes the class: only proxy admins decide where a vector store sends traffic or what signs it. It started as a denylist of endpoint keys and four review rounds each found a missing one (the last: `vertex_credentials`, a workload identity config that makes google-auth read a local file and post it to a caller-chosen token_url), so it is now an allowlist. On `/vector_store/new`, `/vector_store/update` and RAG ingest a non-admin may only set, change or clear `NON_ADMIN_VECTOR_STORE_PARAMS` (`litellm/types/vector_stores.py`): every `mongodb_*` data and retrieval setting, `litellm_embedding_model`/`embedding_model`, per-ingest options (`vector_store_id`, `custom_metadata`, `file_description`, `data_source_id`, `wait_for_ingestion`, `ingestion_timeout`, `max_embedding_requests_per_min`, `wait_for_import`, `import_timeout`, `ttl_days`), and selectors that only reach a request path or body (`s3_prefix`, `vector_bucket_name`, `index_name`, `dimension`, `distance_metric`, `non_filterable_metadata_keys`, `vertex_collection_id`, `vertex_engine_id`, `azure_search_vector_field`, `milvus_*`, `valkey_text_field`/`valkey_embedding_field`). Values must be scalars or lists of scalars, except `custom_metadata` and `mongodb_hybrid_weights`. Anything else, including api_base, api_key, credentials, tokens, regions, projects, `s3_bucket` (a hostname), `ssl_verify`, `litellm_embedding_config` and `litellm_credential_name`, is a 403 naming the keys; keeping a saved value is fine, and a non-admin can't change a store's provider or add an `os.environ/` reference. A managed store pins `VECTOR_STORE_ENDPOINT_KEYS` (now including vertex_credentials, vertex_ai_credentials, azure_ad_token, azure_scope, aws_session_tags, aws_bedrock_project_id, s3_endpoint_url, sagemaker_base_url, deployment_url) on every search, RAG query and ingest, and non-admin bodies on search, RAG query, `/v1/vector_stores` create/update and vector store file create/update can't carry them. The key names shared with the proxy-wide request-body ban live in one constant, `CLIENT_ENDPOINT_AND_CREDENTIAL_PARAMS`, with the ban's contents unchanged. Non-admins keep creating, ingesting into and searching MongoDB stores on the deployment sidecar defaults, and editing their data settings. `company_handbook` (admin-saved sidecar api_base, `os.environ/MONGODB_SIDECAR_API_KEY`) is unaffected. Open: rows a non-admin saved before this fix can still hold keys outside the allowlist and are loaded as-is, so audit `LiteLLM_ManagedVectorStoresTable` on deployments. `/v1/indexes` and index lifecycle passthroughs were already admin-only, and `/config/update` is admin-only |
| Security fix (2026-09-22): embedding model credential redirect | A non-admin could save `litellm_embedding_model` (or `embedding_model`) as `huggingface/https://<their host>`, and because the proxy's vector store embedding fell back to calling litellm directly for a model the router did not serve, every search sent `Bearer $HUGGINGFACE_API_KEY` to that host. On a proxy, store query embeddings now run only through the router: `RouterVectorStoreEmbeddingExecutor` fails with a 400 (`embedding model X is not configured on this proxy`) instead of falling back, and RAG ingest already embedded and ran OCR through the router when one exists. The direct path stays for SDK use with no router. At write time `/vector_store/new`, `/vector_store/update` (changed values only) and RAG ingest reject, for non-admins, a store `litellm_embedding_model`/`embedding_model` or an `ingest_options.embedding.model`/`ingest_options.ocr.model` the router does not serve (400) or that the caller's key, team, team membership or project may not call (403, via `can_key_call_resolved_model`, the same check the auth layer runs for /embeddings and /chat). `litellm_credential_name` in `/v1/vector_stores` create/update and vector store file create/update bodies is now a 403 for non-admins through the same `assert_proxy_admin_for_request_endpoints` helper. Search behaviour, unchanged on purpose: a search embeds the query with the model the store's writer saved, on the router deployment, and does not check that model against the searching key's allowed models, so any key that can access a store can spend that model through it. Behaviour change: an S3 Vectors store with no embedding model now needs the router to serve `text-embedding-3-small` (LIT-6750 had it fall back to the SDK), and a non-admin Bedrock store's `embedding_model` must be a router-served name. `company_handbook` with `text-embedding-3-small` served by the router keeps working for search and ingest. Open: an admin's ingest with an unserved model reports `status: failed` in a 200 body (the pipeline's existing contract), and router wildcard deployments like `huggingface/*` still serve any matching name with the deployment's key, so admins should avoid provider wildcards on proxies with non-admin vector store users |
...
### Dashboard verification notes (2026-09-21)

Every delivered item was exercised in the browser at `localhost:3000` against the local proxy, the local sidecar and
Atlas Local, except one step: running Test connection from inside the add dialog needs the sidecar API key typed into
a form field, which the agent doing the work would not do. That path is covered by the integration suite instead, and
the same checklist component is verified live from the info page, where the proxy reuses the saved secret and nothing
has to be retyped.

Filtered search was confirmed end to end: an unfiltered query returned three chunks and the same query with
`metadata.department eq hr` returned one, built by the filter builder from the store's own `mongodb_filter_fields`.

### Upstream UX defects found while testing (candidates for PR 4/5 or separate small PRs)

1. **Fixed in `4b446c7a`.** Config-registered stores vanished after a list. With a database connected, `GET /vector_store/list` treats the DB as the source of truth and evicts in-memory stores that have no DB row, including ones from `vector_store_registry` in config. Opening the dashboard page triggers this. Fix: only evict stores that originated from the DB.
2. **Fixed in `c4f5df97`.** Original filenames were discarded. `_secure_uploaded_file` replaced the upload name with a random safe name and nothing kept the original, so search results and the Files column showed `debca7d….txt`. Fix: `validate_upload` also returns a sanitized display name, which the proxy threads through `aingest` as `display_filename`. That parameter reaches every provider's ingestion `store()` signature for interface consistency, but today only `mongodb_ingestion` (stores it on every chunk) and the proxy's `vector_store_metadata` file entry actually use it; the other providers accept and ignore it, keeping the generated name they use as an S3 key, vector key or file-API name.
3. **Fixed in `62555e3f`.** Failed ingests created a junk store row. A failed `RAGIngestResponse` carries `vector_store_id: ""`, which passed the `None`/type guard in `_save_vector_store_to_db_from_rag_ingest`, so a row with an empty ID was written. Fix: skip empty strings and `status == "failed"`. `GET /vector_store/list` already skips falsy ids when it builds its store map, so a junk row written by an earlier build does not break the page; a test pins that rather than changing it.
4. **Fixed in `c470e4bd`.** The info endpoint served a stale in-memory copy. `POST /vector_store/info` returned `vector_store_metadata: null` right after two successful ingests until a list call re-synced memory from the DB. Fix: read the row first and fall back to memory only when there is no row, which is how config-registered stores are served. Both paths now return the same `LiteLLM_ManagedVectorStoresTable` shape, with metadata parsed whether the row holds a dict or a JSON string and `litellm_params` redacted as before.
...
6. **Fixed in `4b446c7a`, then narrowed after a security review (see Status).** `os.environ/` references in vector store params were never resolved. Search tools resolve them at load (`proxy_server.py`, "Handle os.environ/ variables in litellm_params") but vector stores do not, so a saved `api_key: os.environ/MONGODB_SIDECAR_API_KEY` is sent to the provider literally. Workaround: omit `api_key` and rely on the provider's env fallback. Fix: resolve references in the shared registry load path for every provider.
7. **Fixed in `4b446c7a` (also fixes defect 1).** The 10-second DB sync never updated or removed stores. `_init_vector_stores_in_db` calls `add_vector_store_to_registry`, which returns early when the id is already present, and nothing deletes ids missing from the DB. On multi-instance Cloud Run, an update or delete performed on one instance leaves the others serving the old registration until restart (seen live: ingest used stale params while Test Connection, which reads the DB, saw the new ones). Workaround: register under a new id. Fix: reconcile DB-sourced entries against the DB set on every sync, keeping config-sourced ones.
8. **Fixed in `08b25fdc`.** Unknown store params without the `mongodb_` prefix were silently ignored (`embedding_model`, `dimensions`, `filter_fields` typos surfaced only as "litellm_embedding_model is required" later). Fix: `reject_unknown_params` now rejects any key that is not a `MongoDBVectorStoreParams` field, a `GenericLiteLLMParams` field, one of the generic store keys, or one of `all_litellm_params`, which is what the SDK and proxy thread through the same mapping. The error names each unknown key and suggests the closest supported one via `difflib`. The managed-store caller options moved to `litellm/types/vector_stores.py` so the provider and the proxy share one definition.
...
2. **Connection section**: on mount, `GET /vector_store/provider_defaults?custom_llm_provider=mongodb` reports whether the deployment already has `MONGODB_SIDECAR_API_BASE`/`MONGODB_SIDECAR_API_KEY` configured (proxy admin only; the key itself is never returned, only whether one is set). When it does, the section shows "Using this deployment's MongoDB sidecar at &lt;api_base&gt;" and collapses Sidecar URL and Sidecar API Key behind an "Override sidecar connection" toggle; collapsed, the form submits neither field and the backend resolves both from the environment. When the deployment has no defaults configured, both fields render plainly as a required per-store connection, same as every other provider. Either way, a **Test connection** button (pattern: `add_model/model_connection_test.tsx` and `search-tools/CreateSearchTools.tsx`) sends `litellm_params` without the two fields when the override is collapsed. Result renders as a checklist with green/amber/red rows, each with a one-line fix, plus "Show details" and a copyable curl. Until the test passes, the sections below stay collapsed with a hint.
