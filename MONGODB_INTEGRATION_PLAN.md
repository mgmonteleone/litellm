...
2. **Fixed in `c4f5df97`.** Original filenames were discarded. `_secure_uploaded_file` replaced the upload name with a random safe name and nothing kept the original, so search results and the Files column showed `debca7d….txt`. Fix: `validate_upload` also returns a sanitized display name, which the proxy threads through `aingest` into the ingestion pipeline (MongoDB stores it on every chunk) and into the `vector_store_metadata` file entry. Storage names are unchanged, so the other providers keep the generated name they use as an S3 key, vector key or file-API name.
