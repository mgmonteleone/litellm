...
6. **Fixed in `4b446c7a`.** `os.environ/` references in vector store params were never resolved. Search tools resolve them at load (`proxy_server.py`, "Handle os.environ/ variables in litellm_params") but vector stores do not, so a saved `api_key: os.environ/MONGODB_SIDECAR_API_KEY` is sent to the provider literally. Workaround: omit `api_key` and rely on the provider's env fallback. Fix: resolve references in the shared registry load path for every provider.
